...
def get_quiz(quiz_id):
    ...
    # TODO: Call the `get_quiz` method with `quiz_id`
    # and store the result in `quiz`
    ...
    if not quiz:
        return jsonify({"message": "This quiz does not exist"}), 200
    else:
        return jsonify({"title": quiz.title, "questions": quiz.questions})
