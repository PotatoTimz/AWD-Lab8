...
def test_create_quiz(mock_create_quiz, client):
    ...
    print(response.json)
    assert response.json == {'message': 'Quiz created', 'quiz_id': [1, 'Quiz created']}

    mock_create_quiz.assert_called_once_with({'questions': [{'answer': '4', 'text': 'What is 2 + 2?'}, {'answer': 'Paris', 'text': 'What is the capital of France?'}], 'title': 'Sample Quiz'})
...
def test_get_quiz(mock_get_quiz, client):
    ...
    print(response.json)
    assert response.json == {'title': 'Sample Quiz', 'questions': [{'text': 'What is 2 + 2?', 'answer': '4'}, {'text': 'What is the capital of France?', 'answer': 'Paris'}]}
...
def test_submit_quiz(mock_evaluate_quiz, client):
    ...
    # INCOMPLETE: Write assertions to check the response
    assert response.status_code == 200
    assert response.json == {"score": 2, "message": "Quiz evaluated successfully"}
